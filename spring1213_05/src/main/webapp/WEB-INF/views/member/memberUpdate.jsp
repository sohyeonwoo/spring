	   ...
	   <form name="m_frm" method="post" action="Do_mwrite">
		   <table>
		     ...
		     <tr>
		       ...
		       <th>취미</th>
		       <td>
		         <input type="checkbox" name="hobby" id="game" value="${hobby }"
		         <c:if test="${fn:contains(mdto.hobby,'game') }">checked</c:if>
		         >
		         
		         <label for="game">게임</label>
		         <input type="checkbox" name="hobby" id="golf" value="${hobby }"
		         <c:if test="${fn:contains(mdto.hobby,'golf') }">checked</c:if>
		         >
		         <label for="golf">골프</label>
		         <input type="checkbox" name="hobby" id="run" value="${hobby }"
		          <c:if test="${fn:contains(mdto.hobby,'run') }">checked</c:if>
		          >
		         <label for="run">조깅</label>
		         <input type="checkbox" name="hobby" id="cook" value="${hobby }"
		          <c:if test="${fn:contains(mdto.hobby,'cook') }">checked</c:if>
		          >
		         <label for="cook">요리</label>
		         <input type="checkbox" name="hobby" id="book" value="${hobby }"
		   ...
		   <button type="button" id="fbtn"><a href="/notice/noticeList">회원정보수정</a></button>
		   <button type="button" onclick="javascript:history.back()"><a href="/">취소</a></button>
	   </form>
	  </div>
