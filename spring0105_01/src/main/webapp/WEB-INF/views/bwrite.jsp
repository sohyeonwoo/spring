...
//여기 부분이 이미지를 첨부하는 부분
  ...
  function uploadImage(file,this_location){ //매개변수 - file : 이미지파일,현재 위치값
	  var fdata = new FormData(); //ajax에서 전송할때 form 형태로 전송
 	  ...
 	  $.ajax({
		 ...
		 data:fdata,
		 cache:false,       //남아 있는 데이터 정보를 정리후 최신화 값을 넣어줌
		 contentType:false, //파일을 보내는 형태 default 값 : application/x.www-form-urlencoded 
		 processData:false, //query String 형태로 보내지 않음 : http://localhost:800/bno=1&..... <<query String 형태
		 enctype:"multipart/form-data",
		 success:function(data){ // 서버에 가서 이미지를 저장후 "파일이름"과 "저장위치" 을 리턴받음
			 					//data:/upload/4896541_1.jpg
